...
|--------------------------------------------------------------------------
|
| Storefront routes are public but rate limited (see AppServiceProvider).
| Account routes sit behind auth:sanctum.
...
Route::prefix('v1')->name('api.v1.')->middleware('throttle:api')->group(function (): void {
    ...
    // Shipping rates — quoted at checkout, before an order exists
    Route::get('/shipping/rates', [ShippingController::class, 'rates'])->name('shipping.rates');

    // Full-text search across frames + art
    ...
    Route::post('/enquiries', [EnquiryController::class, 'store'])->middleware('throttle:forms')->name('enquiries.store');
    ...
    Route::post('/webhooks/shiprocket', [ShippingController::class, 'webhook'])->middleware('throttle:webhooks')->name('webhooks.shiprocket');

    // Auth — guest endpoints, tightly rate limited
